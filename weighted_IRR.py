...
class iplb(object):
    ...
    def _pick_server(self, key, inport):
        ...
        weights=getweights()
        ipserver=0
        while(ipserver==0):                                         #until we get server for load balancing 
        
            rounds = (rounds) % max(weights)                       #divided into no. of cycles of max weight.
            turn=(turn)%len(weights)

        #print("rounds = ",rounds+1)
            if turn==0:                                          #first server's turn and check if it can accomodate new connection else skip.
                if rounds+1 <= weights[turn]:
                    for x in self.total_connection:
                        if str(x) == '10.0.0.1':
                            ipserver = x
                            print("Best available Server:", ipserver)

                turn=turn+1
                    
            if turn==1:                                              #second server's turn and check if it can accomodate new connection else skip.
                if rounds+1 <= weights[turn]:
                    for x in self.total_connection:
                        if str(x) == '10.0.0.2':
                            ipserver = x
            ...
            else:                                                   #third server's turn and check if it can accomodate new connection else skip.
                if rounds+1 <= weights[turn] :
                    for x in self.total_connection:
                        if str(x) == '10.0.0.3':
                            ipserver = x
                ...
                rounds=rounds+1                                 #next cycle start 
                turn=0
