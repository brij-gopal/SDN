...
class iplb(object):
    ...
    def __init__(self, connection, service_ip, servers=[]):                #(iplb, event.connection, IPAddr(ip), servers)
        self.service_ip = IPAddr(service_ip)
        self.servers = [IPAddr(a) for a in servers]
        self.con = connection               #getting the connection object (self.con =', <pox.openflow.of_01.Connection object at 0xb5c5f26c>)
                                        
        self.mac = self.con.eth_addr      #mac address of load balancer switch ('self.con.eth_addr =', EthAddr('00:00:00:00:00:01'))
        ...
        for ip in servers:
            self.total_connection[ip] = 0    # creating dictionary 'total_connection'
                                              #('self.total_connection= ', {IPAddr('10.0.0.1'): 0, IPAddr('10.0.0.3'): 0, IPAdddr('10.0.0.2'): 0})                                                                                       r('10.0.0.2'): 0})
    ...
    def _handle_PacketIn(self, event):           #packet arrive to switch and send packet in event to controller
        inport = event.port
        packet = event.parsed
        ...
        tcpp = packet.find('tcp')       #A packet object's find() method can be used to find a specific encapsulated packet by the desired type name (e.g., "icmp") 
                                        # or its class (e.g., pkt.ICMP).  If the packet object does not encapsulate a packet of the requested type, find() returns None.
        if not tcpp:
            ...
            if arpp:
                ...
                if arpp.opcode == arpp.REPLY:
                    if arpp.protosrc in self.outstanding_probes:
                        ...
                        if (self.live_servers.get(arpp.protosrc, (None, None))           # { IP : (MAC,port)}
                                == (arpp.hwsrc, inport)):
                            # Ah, nothing new here.
                            pass
                        else:
        ...
        if ipp.srcip in self.servers:
            ...
            entry = self.memory.get(key)                        #{(srcip,dstip,srcport,dstport) : MemoryEntry}
        ...
        # Incoming packet from client
        elif ipp.dstip == self.service_ip: #--------------------------------start load balancing module ---------------
            ...
            if entry is None or entry.server not in self.live_servers:
                ...
                server = self._pick_server(key, inport)    #call to picks_erver() function 
...
def launch(ip, servers):                            #storing virtual ip and servers ip from cmd 
    servers = servers.replace(",", " ").split()     #('servers=', [IPAddr('10.0.0.1'), IPAddr('10.0.0.2'), IPAddr('10.0.0.3')])
    servers = [IPAddr(x) for x in servers]
    ip = IPAddr(ip)                                 #('virtual_ip=', IPAddr('10.0.1.1'))
...
# This component also adds the ARP table to the interactive console as "arp" – allowing you to interactively query and modify it.
...
#proto.arp_responder --192.168.0.1=00:00:00:00:00:01 --192.168.0.2=00:00:00:00:00:02 
    ...
    def _handle_ConnectionUp(event):
        global _dpid
        if _dpid is None:                               #only one switch will be consider for load balancing if multiple switches are present in topology
            log.info("IP Load Balancer Ready.")
            core.registerNew(iplb, event.connection, IPAddr(ip), servers)  
            _dpid = event.dpid

        if _dpid != event.dpid:                       #rest all other switches ignored.
            log.warn("Ignoring switch %s", event.connection)
        else:
            log.info("Load Balancing on %s", event.connection)          #starting load balancing on selected switch 
